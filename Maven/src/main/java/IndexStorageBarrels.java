        ...
        if (copy.exists() && searchResultsBin.exists()) {
            ...
            if (resultados_pesquisa == null) {
                if (searchResultsBin.exists())
                    resultados_pesquisa = (ConcurrentHashMap<Integer, ArrayList<infoURL>>) FileOps
                            .readFromDisk(searchResultsBin);
                else
    ...
    synchronized public ArrayList<infoURL> resultadoPesquisa(String termo_pesquisa, Integer id_client)
        ...
        new Thread(() -> {
            if (countIterations > 25) {
                lockFile1.lock();
                FileOps.writeToDisk(new File(String.format("./ISB/searchResults_%s.bin", barrelName)),
                        (resultados_pesquisa));
                lockFile1.unlock();
                countIterations = 0;
            }
