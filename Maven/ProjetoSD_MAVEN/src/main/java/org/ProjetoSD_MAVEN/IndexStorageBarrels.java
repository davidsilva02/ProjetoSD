                ...
                if(obj!=null){
                    ...
                    new Thread(() -> {
                        if(countIterations > 24){
                            lockFile.lock();
                            FileOps.ToDisk(new File(String.format("./ISB/index_%s.bin",barrelName)),ind);
                            FileOps.ToDisk(new File(String.format("./ISB/urls_%s.bin",barrelName)),urls);
                            lockFile.unlock();
                            countIterations = 0;
                        }
                    }).start();
    ...
    synchronized public ArrayList<infoURL> resultadoPesquisa(String termo_pesquisa, Integer id_client) throws RemoteException {
        ...
        new Thread(() -> {
            if(countIterations > 25){
                lockFile1.lock();
                FileOps.ToDisk(new File(String.format("./ISB/searchResults_%s.bin",barrelName)), (resultados_pesquisa));
                lockFile1.unlock();
                countIterations = 0;
            }
        }).start();
