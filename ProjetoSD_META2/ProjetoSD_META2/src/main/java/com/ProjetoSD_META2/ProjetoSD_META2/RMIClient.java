...
public class RMIClient {
    ...
    public String  getSearch(@RequestParam String search) {
        ...
        ArrayList <infoURL> res=null;
        try {
            res=server.resultadoPesquisa(search,1);
        } catch (RemoteException e) {
    ...
    public String insertURL(@RequestParam String url) {
        //chamar metodo RMI para inserir url
        String res="Existiu um problema ao inserir url";
        try {
            server.putURLClient(url);
            res= String.format("%s inserido com sucesso!\n",url);
        } catch (RemoteException e) {
            throw new RuntimeException(e);
        }
